...
private let transcriptionFeatureLogger = VoixeLog.transcription
  ...
  func handleStartRecording(_ state: inout State) -> Effect<Action> {
    ...
    return .merge(
      ...
      .run { [sleepManagement, preventSleep = state.hexSettings.preventSystemSleep] _ in
        ...
        if preventSleep {
          await sleepManagement.preventSleep(reason: "Voixe Voice Recording")
        }
...
private extension TranscriptionFeature {
  func handleTranscriptionResult(
    ...
    let refineSettings = state.hexSettings.refine
    let shouldRefine = refineSettings.isEnabled && !state.isRemappingScratchpadFocused
    if shouldRefine {
    ...
    return .run { [llmRefinement] send in
      ...
      if shouldRefine {
        do {
          let refined = try await llmRefinement.refine(
            modifiedResult,
            sourceAppName,
            sourceAppBundleID,
            refineSettings
          ...
          finalResult = refined
          transcriptionFeatureLogger.info("Applied LLM refine pass")
        } catch {
          transcriptionFeatureLogger.error("Refine failed: \(error.localizedDescription, privacy: .public)")
          if !refineSettings.fallbackOnError {
            await send(.refineFinished)
            await send(.transcriptionError(error, audioURL))
            return
          }
          // Fall through with the unrefined text.
        }
        await send(.refineFinished)
      }
...
struct TranscriptionView: View {
  ...
  var status: VoixeIndicatorView.Status {
    if store.isRefining {
      return .refining
...
private enum ForceQuitCommandDetector {
  static func matches(_ text: String) -> Bool {
    ...
    return normalized == "force quit voixe now"
      || normalized == "force quit voixe"
      || normalized == "force quit hex now"
      || normalized == "force quit hex"
  }
