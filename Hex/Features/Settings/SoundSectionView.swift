...
private func volumePercentage(for actualVolume: Double) -> Double {
	guard VoixeSettings.baseSoundEffectsVolume > 0 else { return 0 }
	let ratio = actualVolume / VoixeSettings.baseSoundEffectsVolume
	return max(0, min(1, ratio))
}

private func actualVolume(fromPercentage percentage: Double) -> Double {
	let clampedPercentage = max(0, min(1, percentage))
	return clampedPercentage * VoixeSettings.baseSoundEffectsVolume
}
