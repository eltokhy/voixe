# Voixe by Enginecy – Dev Notes for Agents

This file provides guidance for coding agents working in this repo.

## Project Overview

Voixe is a macOS menu bar application for on‑device voice‑to‑text. It supports Whisper (Core ML via WhisperKit) and Parakeet TDT v3 (Core ML via FluidAudio). Users activate transcription with hotkeys; text can be auto‑pasted into the active app.
...
5. **Permissions**: Requires audio input and automation entitlements (see `Hex/Hex.entitlements`)

6. **Logging**: All diagnostics should use the unified logging helper `VoixeLog` (`VoixeCore/Sources/VoixeCore/Logging.swift`). Pick an existing category (e.g., `.transcription`, `.recording`, `.settings`) or add a new case so Console predicates stay consistent. Avoid `print` and prefer privacy annotations (`, privacy: .private`) for anything potentially sensitive like transcript text or file paths.
...
- WhisperKit models
  - `~/Library/Application Support/com.enginecy.voixe/models/argmaxinc/whisperkit-coreml/<model>`
- Parakeet (FluidAudio)
  - We set `XDG_CACHE_HOME` on launch so Parakeet caches under the app container:
  - `~/Library/Containers/com.enginecy.voixe/Data/Library/Application Support/FluidAudio/Models/parakeet-tdt-0.6b-v3-coreml`
...
- FluidAudio: `https://github.com/FluidInference/FluidAudio.git` (link `FluidAudio` to the app target)
...
```
XDG_CACHE_HOME = ~/Library/Containers/com.enginecy.voixe/Data/Library/Application Support/com.enginecy.voixe/cache
```
...
- Parakeet not detected: ensure it resides under the container path above; downloading from Voixe places it correctly.
