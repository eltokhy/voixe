...
struct AppFeature {
  ...
  enum Action: BindableAction {
    ...
    /// Toggle Refine on from the onboarding wizard, kicking off bootstrap.
    case enableRefineFromOnboarding
  }
  ...
  var body: some ReducerOf<Self> {
    ...
    Reduce { state, action in
      ...
      case .transcription(.modelMissing):
        VoixeLog.app.notice("Model missing - activating app and switching to settings")
        state.activeTab = .settings
        state.settings.shouldFlashModelSection = true
        return .run { send in
          await MainActor.run {
            VoixeLog.app.notice("Activating app for model missing")
      ...
      case .completeFirstRun:
        state.$hexSettings.withLock { $0.hasCompletedFirstRun = true }
        return .none

      case .enableRefineFromOnboarding:
        return .send(.settings(.setRefineEnabled(true)))
...
struct AppView: View {
  ...
  var body: some View {
    ...
    .sheet(isPresented: Binding(
      get: { !store.hexSettings.hasCompletedFirstRun },
      set: { isPresented in
        if !isPresented { store.send(.completeFirstRun) }
      }
    )) {
