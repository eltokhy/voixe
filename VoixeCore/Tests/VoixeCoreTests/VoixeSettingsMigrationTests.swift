...
final class VoixeSettingsMigrationTests: XCTestCase {
	func testV1FixtureMigratesToCurrentDefaults() throws {
		...
		XCTAssertEqual(decoded.soundEffectsVolume, VoixeSettings.baseSoundEffectsVolume)
	...
	func testInitNormalizesDoubleTapOnlyWhenLockDisabled() {
		let settings = VoixeSettings(useDoubleTapOnly: true, doubleTapLockEnabled: false)
	...
	private func loadFixture(named name: String) throws -> Data {
		guard let url = Bundle.module.url(
			forResource: name,
			withExtension: "json",
			subdirectory: "Fixtures/VoixeSettings"
		) else {
