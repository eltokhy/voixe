...
private let settingsLogger = VoixeLog.settings
...
struct SettingsFeature {
  ...
  struct State {
    ...
    // Refine
    var availableRefineModels: [String] = []
    var refineConnectionState: RefineConnectionState = .unknown
    var refineModelsError: String?
    var refineModelDownload = RefineModelDownloadFeature.State()
  }

  enum RefineConnectionState: Equatable, Sendable {
    case unknown
    case testing
    case ok
    case failed
  ...
  enum Action: BindableAction {
    ...
    // Refine (local LLM)
    case setRefineEnabled(Bool)
    case setRefineRuntimeMode(RefineRuntimeMode)
    case setRefineEndpoint(String)
    case setRefineModel(String)
    case setRefineSystemPrompt(String)
    case resetRefineSystemPrompt
    case setRefineTemperature(Double)
    case setRefineFallbackOnError(Bool)
    case loadRefineModels
    case refineModelsLoaded([String])
    case refineModelsFailed(String)
    case testRefineConnection
    case refineConnectionTested(Bool)
    case refineModelDownload(RefineModelDownloadFeature.Action)
  }
  ...
  @Dependency(\.llmRefinement) var llmRefinement
  ...
  var body: some ReducerOf<Self> {
    ...
    Scope(state: \.refineModelDownload, action: \.refineModelDownload) {
      RefineModelDownloadFeature()
    }
    ...
    Reduce { state, action in
      ...
      case let .setRefineEnabled(enabled):
        state.$hexSettings.withLock { $0.refine.isEnabled = enabled }
        if enabled {
          if state.hexSettings.refine.runtimeMode == .bundled {
            return .send(.refineModelDownload(.ensureBootstrap))
          }
          if state.availableRefineModels.isEmpty {
            return .send(.loadRefineModels)
          }
        }
        return .none

      case let .setRefineRuntimeMode(mode):
        state.$hexSettings.withLock { $0.refine.runtimeMode = mode }
        switch mode {
        case .bundled where state.hexSettings.refine.isEnabled:
          return .send(.refineModelDownload(.ensureBootstrap))
        case .ollama where state.availableRefineModels.isEmpty:
          return .send(.loadRefineModels)
        default:
          return .none
        }

      case .refineModelDownload:
        return .none

      case let .setRefineEndpoint(endpoint):
        state.$hexSettings.withLock { $0.refine.endpoint = endpoint }
        state.refineConnectionState = .unknown
        state.availableRefineModels = []
        return .none

      case let .setRefineModel(model):
        state.$hexSettings.withLock { $0.refine.model = model }
        return .none

      case let .setRefineSystemPrompt(prompt):
        state.$hexSettings.withLock { $0.refine.systemPrompt = prompt }
        return .none

      case .resetRefineSystemPrompt:
        state.$hexSettings.withLock { $0.refine.systemPrompt = RefineSettings.defaultSystemPrompt }
        return .none

      case let .setRefineTemperature(value):
        state.$hexSettings.withLock { $0.refine.temperature = value }
        return .none

      case let .setRefineFallbackOnError(enabled):
        state.$hexSettings.withLock { $0.refine.fallbackOnError = enabled }
        return .none

      case .loadRefineModels:
        let endpoint = state.hexSettings.refine.endpoint
        state.refineModelsError = nil
        return .run { [llmRefinement] send in
          do {
            let models = try await llmRefinement.listModels(endpoint)
            await send(.refineModelsLoaded(models))
          } catch {
            await send(.refineModelsFailed(error.localizedDescription))
          }
        }

      case let .refineModelsLoaded(models):
        state.availableRefineModels = models
        state.refineModelsError = nil
        // If the currently selected model is absent, silently fall back to the first entry.
        if !models.isEmpty, !models.contains(state.hexSettings.refine.model) {
          state.$hexSettings.withLock { $0.refine.model = models[0] }
        }
        return .none

      case let .refineModelsFailed(message):
        state.availableRefineModels = []
        state.refineModelsError = message
        return .none

      case .testRefineConnection:
        let endpoint = state.hexSettings.refine.endpoint
        state.refineConnectionState = .testing
        return .run { [llmRefinement] send in
          let ok = await llmRefinement.ping(endpoint)
          await send(.refineConnectionTested(ok))
          if ok {
            await send(.loadRefineModels)
          }
        }

      case let .refineConnectionTested(ok):
        state.refineConnectionState = ok ? .ok : .failed
        return .none
