...
private let appLogger = VoixeLog.app
private let cacheLogger = VoixeLog.caches

class VoixeAppDelegate: NSObject, NSApplicationDelegate {
	...
	private func startLifecycleTasksIfNeeded() {
		Task { @MainActor in
			await VoixeApp.appStore.send(.task).finish()
		}
	}
	...
	func presentMainView() {
		...
		let transcriptionStore = VoixeApp.appStore.scope(state: \.transcription, action: \.transcription)
	...
	func presentSettingsView() {
		...
		let settingsView = AppView(store: VoixeApp.appStore)
