...
public struct VoixeSettings: Codable, Equatable, Sendable {
	public static let defaultPasteLastTranscriptHotkey = HotKey(key: .v, modifiers: [.option, .shift])
	public static let baseSoundEffectsVolume: Double = VoixeCoreConstants.baseSoundEffectsVolume
	...
	public init(
		...
		soundEffectsVolume: Double = VoixeSettings.baseSoundEffectsVolume,
		...
		minimumKeyTime: Double = VoixeCoreConstants.defaultMinimumKeyTime,
		...
		pasteLastTranscriptHotkey: HotKey? = VoixeSettings.defaultPasteLastTranscriptHotkey,
		hasCompletedModelBootstrap: Bool = false,
		hasCompletedStorageMigration: Bool = false,
		hasCompletedRefineBootstrap: Bool = false,
		hasCompletedFirstRun: Bool = false,
		wordRemovalsEnabled: Bool = false,
		wordRemovals: [WordRemoval] = VoixeSettings.defaultWordRemovals,
		...
		self.hasCompletedRefineBootstrap = hasCompletedRefineBootstrap
	...
	public init(from decoder: Decoder) throws {
		self.init()
		let container = try decoder.container(keyedBy: VoixeSettingKey.self)
		for field in VoixeSettingsSchema.fields {
	...
	public func encode(to encoder: Encoder) throws {
		var container = encoder.container(keyedBy: VoixeSettingKey.self)
		for field in VoixeSettingsSchema.fields {
...
private enum VoixeSettingKey: String, CodingKey, CaseIterable {
	...
	case hasCompletedRefineBootstrap
...
private struct SettingsField<Value: Codable & Sendable> {
	let key: VoixeSettingKey
	let keyPath: WritableKeyPath<VoixeSettings, Value>
	let defaultValue: Value
	let decodeStrategy: (KeyedDecodingContainer<VoixeSettingKey>, VoixeSettingKey, Value) throws -> Value
	let encodeStrategy: (inout KeyedEncodingContainer<VoixeSettingKey>, VoixeSettingKey, Value) throws -> Void

	init(
		_ key: VoixeSettingKey,
		keyPath: WritableKeyPath<VoixeSettings, Value>,
		default defaultValue: Value,
		decode: ((KeyedDecodingContainer<VoixeSettingKey>, VoixeSettingKey, Value) throws -> Value)? = nil,
		encode: ((inout KeyedEncodingContainer<VoixeSettingKey>, VoixeSettingKey, Value) throws -> Void)? = nil
	) {
		self.key = key
...
private struct AnySettingsField {
	let key: VoixeSettingKey
	let decode: (KeyedDecodingContainer<VoixeSettingKey>, inout VoixeSettings) throws -> Void
	let encode: (VoixeSettings, inout KeyedEncodingContainer<VoixeSettingKey>) throws -> Void

	func decode(into settings: inout VoixeSettings, from container: KeyedDecodingContainer<VoixeSettingKey>) throws {
		try decode(container, &settings)
	}

	func encode(_ settings: VoixeSettings, into container: inout KeyedEncodingContainer<VoixeSettingKey>) throws {
		try encode(settings, &container)
	}
}

private enum VoixeSettingsSchema {
	static let defaults = VoixeSettings()
	...
	nonisolated(unsafe) static let fields: [AnySettingsField] = [
		...
		SettingsField(.hasCompletedRefineBootstrap, keyPath: \.hasCompletedRefineBootstrap, default: defaults.hasCompletedRefineBootstrap).eraseToAny(),
		SettingsField(.hasCompletedFirstRun, keyPath: \.hasCompletedFirstRun, default: defaults.hasCompletedFirstRun).eraseToAny(),
